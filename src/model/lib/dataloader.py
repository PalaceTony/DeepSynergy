...
def get_dataloader(args, device):
    if args.model == "deepSynergy":
        with open(args.data_file, "rb") as file:
            X_tr, X_val, X_train, X_test, y_tr, y_val, y_train, y_test = pickle.load(
                file
            )
        ...
        X_val, y_val = torch.tensor(X_val, dtype=torch.float32).to(
            device
        ), torch.tensor(y_val, dtype=torch.float32).to(device)
        X_train, y_train = torch.tensor(X_train, dtype=torch.float32).to(
            device
        ), torch.tensor(y_train, dtype=torch.float32).to(device)
        X_test, y_test = torch.tensor(X_test, dtype=torch.float32).to(
            device
        ...
        final_train_loader = DataLoader(
            final_train_dataset, batch_size=args.batch_size, shuffle=True
        )
        ...
        def to_tensor_and_device(arrays):
            return [
                torch.tensor(array, dtype=torch.float32).to(device) for array in arrays
            ]
        ...
        y_tr, y_val, y_train, y_test = to_tensor_and_device(
