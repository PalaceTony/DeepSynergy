...
class Trainer:
    ...
    def train_epoch(self):
        self.model.train()
        train_loss = 0
        for drug_A, drug_B, cell_line, labels in self.train_loader:
    ...
    def validate(self):
        ...
        val_loss = 0
        with torch.no_grad():
            for drug_A, drug_B, cell_line, labels in self.val_loader:
                outputs = self.model(drug_A, drug_B, cell_line)
    ...
    def test(self):
        ...
        y_pred = []
        with torch.no_grad():
            for drug_A, drug_B, cell_line, labels in self.test_loader:
                outputs = self.model(drug_A, drug_B, cell_line)
